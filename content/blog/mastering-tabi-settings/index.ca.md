...
tabi té tres formats de data: `long_date_format`, `short_date_format` i `archive_date_format`. El format curt s'utilitza a les metadades d'una publicació, mentre que el format llarg s'utilitza al llistar les publicacions (és a dir, a la [secció de blog](/ca/blog/) o a la [pàgina principal](/ca/)). El format d'arxiu s'utilitza per mostrar el dia i el mes a la pàgina d'arxiu.

En anglès, els valors predeterminats són "6th July 2049" per al format llarg i "6th Jul 2049" per al format curt. Per a altres idiomes, el predeterminat és `"dd MMMM y"` per al format llarg i `"d MMM y"` per al format curt. El format d'arxiu predeterminat universal és `"dd MMM"`.

Els formats de data utilitzen [patrons UTS-35](https://unicode.org/reports/tr35/tr35-dates.html#Date_Field_Symbol_Table). Per exemple, `d`, `dd`, `MMM`, `MMMM` i `y` produeixen un dia numèric, un dia amb zero inicial, un mes abreujat, un mes complet i l'any. Envolta el text literal amb apòstrofs, com a `d 'de' MMMM 'de' y`. Els noms textuals dels mesos i dels dies de la setmana segueixen la configuració regional de l'idioma actual.
...
Això permet que cada idioma triï l'ordre, la puntuació i el nivell de detall.
