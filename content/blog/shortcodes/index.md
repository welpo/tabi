...
date = 2023-02-19
updated = 2026-09-13
description = "This theme includes some useful custom components that you can use to enhance your posts. Whether you want to display images that adapt to light and dark themes, or format a professional-looking reference section, these custom components have got you covered."
aliases = ["tags/shortcodes"]
...
{{< admonition type="info" text="tabi's reusable content helpers are Tera components. Use the self-closing or block forms shown in this guide." />}}
...
2. Use the `mermaid` component to define your diagram in your posts. For example:
...
The Mermaid component supports two parameters:
...
All image components support absolute paths, relative paths, and remote sources in the `src` parameter.

All image components have these optional parameters:
...
```
{% raw %}{{< dual_theme_image light_src="img/paris_day.webp" dark_src="img/paris_night.webp" alt="The Eiffel tower" />}}{% endraw %}
```
...
```
{% raw %}{{< invertible_image src="img/graph.webp" alt="Invertible graph" />}}{% endraw %}
```
...
```
{% raw %}{{< dimmable_image src="img/desert_by_oskerwyld.webp" alt="Photograph of a desert, heavenly sky" />}}{% endraw %}
```
...
```
{% raw %}{{< image_hover default_src="img/before.webp" hovered_src="img/after.webp" default_alt="Edited picture" hovered_alt="Original shot" />}}{% endraw %}
```
...
```
{% raw %}{{< image_toggler default_src="img/mojave_day.webp" toggled_src="img/mojave_night.webp" default_alt="Mojave during the day" toggled_alt="Mojave at night" />}}{% endraw %}
```
...
All other image components can be made into full-width by setting the optional parameter `full_width` to `true`.
...
```
{% raw %}{{< full_width_image src="img/amsterdam_by_oskerwyld.webp" alt="Photograph of a canal in Amsterdam" />}}{% endraw %}
```
...
This component allows you to add extra [iine.to](https://iine.to) buttons to your posts, like this:
...
```
{% raw %}{{< iine icon="heart" slug="/post/my-post-slug/like" label="Like this post" />}}{% endraw %}
```

The component takes the following optional parameters:
...
If you set the `name` to a URL (i.e. it starts with `http` or `https`), you can turn it into a clickable link. This is particularly useful when used in conjunction with the [remote text component](#remote-text).
...
- Content can be provided via `text` parameter or between component tags

#### Usage

{{< admonition type="warning" text="Place the aside component on its own line to prevent formatting issues." />}}

Using the `text` parameter:

```
{% raw %}{{< aside text="*Sidenote* comes from Latin *nota* ('mark') + Old English *síde* ('side')." />}}{% endraw %}
```
...
Embed text from a remote URL or a local file. To display the path or URL on the code block, see the [show source or path component](#show-source-or-path).
...
- **Files access**: As it uses Zola's [`load_data`](https://www.getzola.org/documentation/templates/overview/#load-data), local files must be inside the Zola directory—see [File searching logic](https://www.getzola.org/documentation/templates/overview/#file-searching-logic). The component supports both relative and absolute paths.
...
```
{% raw %}{{< remote_text src="path/to/file.txt" />}}{% endraw %}
```

Display lines 3 to 7 (both inclusive) of a local file:

```
{% raw %}{{< remote_text src="path/to/file.txt" start={3} end={7} />}}{% endraw %}
```

### Admonitions

Bring attention to information with these admonition/alert components. They come in five `type`s: `note`, `tip`, `info`, `warning`, and `danger`.
...
```md
{% raw %}{{< admonition type="danger" icon="tip" title="An important tip" text="Stay hydrated~" />}}{% endraw %}
```

2. With a content body:

```md
{% raw %}{% <admonition type="danger" icon="tip" title="An important tip"> %}{% endraw %}
Stay hydrated~
...
This component allows you to display both the translated and original text for a quote. The quotation marks will be added automatically:
...
```
{% raw %}{{< multilingual_quote original="Qué sosiego, ir por la vida en silencio, saludando sólo a los amigos." translated="What tranquillity, to go through life in silence, greeting only friends." author="Francisco Umbral" />}}{% endraw %}
```
...
This component allows you to blur text until the user clicks on it. Like this: Goldfish have a memory span of a few {{< spoiler text="months. Yes, [really](https://en.wikipedia.org/wiki/Goldfish#Cognitive_abilities)." />}}

As you can see, Markdown is rendered. You can even add newlines with `<br>`.

This component has the optional flag `fixed_blur` to blur a fixed placeholder ("SPOILER"), instead of blurring the actual contents. Like this: it is {{< spoiler text="not necessary" fixed_blur={true} />}} to wait 24 hours before filing a missing person report.

#### Usage

```
{% raw %}{{< spoiler text="text to hide" fixed_blur={false} />}}{% endraw %}
```
...
Use this component if you want to have a wider table, paragraph, code block… On desktop, it will take up the width of the header. It will have no effect on mobile, except for tables, which will get a horizontal scroll.
